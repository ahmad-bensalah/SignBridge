...
def get_word_folder(word: str) -> Path:
...
def landmarks_to_dict(results):
    ...
    if results.pose_landmarks:
        data["pose"] = [[lm.x, lm.y, lm.z, lm.visibility]
                        for lm in results.pose_landmarks.landmark]
    if results.left_hand_landmarks:
        data["left_hand"] = [[lm.x, lm.y, lm.z]
                              for lm in results.left_hand_landmarks.landmark]
    if results.right_hand_landmarks:
        data["right_hand"] = [[lm.x, lm.y, lm.z]
    ...
    if video_path:
        ...
        while True:
            ...
            if idx in indices:
                ...
                all_lm.append(landmarks_to_dict(results) if results else {})
            idx += 1
        cap.release()
    else:
        # ── IMAGE SEQUENCE ──
...
words = [w for ws in words_by_cat.values() for w in ws]  # flat list for validation
...
with col_input:
    word_input = st.text_input("Enter a word", placeholder="e.g. 3aslema", label_visibility="collapsed")
with col_btn:
    play = st.button("▶ Play")
with col_speed:
